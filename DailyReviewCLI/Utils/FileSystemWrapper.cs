...
//			}
		...
		private string replaceHyperLink(string line) {
			...
			foreach (Match m in matches) {
				var groups = m.Groups;
				var link = groups[2].Value;
				var newLink = link;
				var text = groups[1].Value;
				var linkParts = link.Split(@"/".ToCharArray()); 
				/*
 				if (link.StartsWith(@"https://www.evernote.com", StringComparison.CurrentCulture)) {
					newLink = @"evernote:///view/" + linkParts[6] + @"/" + linkParts[4] + @"/" + linkParts[7] + @"/" + linkParts[7];
				}
				*/
				result = result.Replace("[" + text + "](" + link + ")", "<a href=\"" + link + "\" style=\"color: rgb(105, 170, 53);\">" + text + "</a>");
			}
